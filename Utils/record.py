...
def update_result(score, dataset_name, model_name, subject_id, type):
    result_excel = openpyxl.load_workbook(os.path.join(os.getcwd(), 'StatisResult', f'{dataset_name}', 'Result.xlsx'))
    sheet = result_excel[f'{model_name}']

    # if sheet['I'+ str(subject_id+1)].value == None or sheet['I'+ str(subject_id+1)].value < score.AUC:
    if True:
        ...
        # 保存文件
        result_excel.save(os.path.join(os.getcwd(), 'StatisResult', f'{dataset_name}', 'Result.xlsx'))
