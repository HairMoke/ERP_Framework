...
class EvaluateManage:
    def __init__(self, subject_id, dataset_name, model_name):
        ...
        self.number_lib = np.zeros([4200*14, 2])
    ...
    def calculate_metric_score(self):
        ...
        for idx, label in enumerate(self.y):
            y =self.y
            if label == 1:          #这里label1还是0待定，不好说，回头根据测试集试出来
        ...
        BA = round((TP / (TP + FN) + TN / (TN + FP)) / 2  * 100, 2)
        TPR = round(TP / (TP + FN) * 100, 2)
        FPR = round(FP / (TN + FP) * 100, 2)
        P = round(TP / (TP + FP) * 100, 2)
        # P = 0.00
        F1 = round(2 * P * TPR / (P + TPR), 2)
